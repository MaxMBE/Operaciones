...
function Bullets({ text, placeholder, className = "" }: { text: string; placeholder?: string; className?: string }) {
  ...
  // Split by newline first, then by inline bullet marker
  const byLine = text.split("\n").map(s => s.replace(/^•\s*/, "").trim()).filter(Boolean);
  const items = byLine.length > 1 ? byLine : text.split("• ").map(s => s.trim()).filter(Boolean);
  if (items.length <= 1) {
...
export default function ProjectReportPage() {
  ...
  const saved = reportData[projectId] ?? emptyReport();
