...
interface FinancialKPIRecord {
  serviceType: string;
  revenueMonthly: number; revenueYTD: number; revenueProjection: number;
  costMonthly: number;    costYTD: number;    costProjection: number;
  fteMandays: number;
}

interface FinancialKPIPayload {
  months: Record<string, FinancialKPIRecord[]>;
  extraServices: string[];
}

const EMPTY_KPI_RECORD = (svc: string): FinancialKPIRecord => ({
  serviceType: svc,
  revenueMonthly: 0, revenueYTD: 0, revenueProjection: 0,
  costMonthly: 0,    costYTD: 0,    costProjection: 0,
  fteMandays: 0,
});
...
function FinancialKPIView() {
  ...
  const fmt = (n: number) => Math.round(n).toLocaleString(lang === "en" ? "en-US" : "es-CL");

  const derivedServices = useMemo(
    () => [...new Set(projects.map(p => p.serviceType).filter(Boolean))].sort() as string[],
    [projects]
  );

  const [payload, setPayload] = useState<FinancialKPIPayload>({ months: {}, extraServices: [] });
  const [selectedMonth, setSelectedMonth] = useState<string>("");
  const [editMode, setEditMode] = useState(false);
  const [draftRows, setDraftRows] = useState<FinancialKPIRecord[]>([]);
  ...
  const allServices = useMemo(
    () => [...new Set([...derivedServices, ...payload.extraServices])].sort(),
    [derivedServices, payload.extraServices]
  );

  // Load from Supabase on mount
  useEffect(() => {
    fetch("/api/settings/financial-kpi")
      .then(r => r.json())
      .then((data: FinancialKPIPayload | null) => {
        if (data && data.months) {
          setPayload(data);
  ...
  const activeRows = useMemo(() => {
    ...
    return payload.months[selectedMonth] ?? allServices.map(EMPTY_KPI_RECORD);
  }, [payload.months, selectedMonth, allServices]);

  const totals = useMemo(() => activeRows.reduce((acc, r) => ({
    revenueMonthly:    acc.revenueMonthly    + r.revenueMonthly,
    revenueYTD:        acc.revenueYTD        + r.revenueYTD,
    revenueProjection: acc.revenueProjection + r.revenueProjection,
    costMonthly:       acc.costMonthly       + r.costMonthly,
    costYTD:           acc.costYTD           + r.costYTD,
    costProjection:    acc.costProjection    + r.costProjection,
    fteMandays:        acc.fteMandays        + r.fteMandays,
  }), { revenueMonthly:0, revenueYTD:0, revenueProjection:0, costMonthly:0, costYTD:0, costProjection:0, fteMandays:0 }),
  [activeRows]);

  const marginColor = (pct: number | null) =>
    pct === null ? "text-muted-foreground"
    : pct >= TMD_TARGET ? "text-emerald-700 font-semibold"
    : pct >= 25 ? "text-amber-600 font-semibold"
    : "text-red-600 font-semibold";

  const tmdColor = (v: number | null) =>
    v === null ? "text-muted-foreground" : v >= 0 ? "text-emerald-700 font-semibold" : "text-red-600 font-semibold";

  function computeRow(r: FinancialKPIRecord) {
    const profitM = r.revenueMonthly - r.costMonthly;
    const profitY = r.revenueYTD - r.costYTD;
    const profitP = r.revenueProjection - r.costProjection;
    const mrgM = r.revenueMonthly > 0 ? Math.round((profitM / r.revenueMonthly) * 100) : null;
    const mrgY = r.revenueYTD > 0 ? Math.round((profitY / r.revenueYTD) * 100) : null;
    const mrgP = r.revenueProjection > 0 ? Math.round((profitP / r.revenueProjection) * 100) : null;
    const tmdM = mrgM !== null ? mrgM - TMD_TARGET : null;
    const tmdY = mrgY !== null ? mrgY - TMD_TARGET : null;
    const tmdP = mrgP !== null ? mrgP - TMD_TARGET : null;
    return { profitM, profitY, profitP, mrgM, mrgY, mrgP, tmdM, tmdY, tmdP };
  }

  const revenueChartData = useMemo(() => activeRows.map(r => ({
    name: r.serviceType.length > 16 ? r.serviceType.slice(0, 14) + "…" : r.serviceType,
    [t.fkpi_col_monthly]:    r.revenueMonthly,
    [t.fkpi_col_ytd]:        r.revenueYTD,
    [t.fkpi_col_projection]: r.revenueProjection,
  })), [activeRows, t]);

  const marginChartData = useMemo(() => activeRows
    .filter(r => r.revenueYTD > 0)
    .map(r => ({
      name: r.serviceType.length > 16 ? r.serviceType.slice(0, 14) + "…" : r.serviceType,
      margin: Math.round(((r.revenueYTD - r.costYTD) / r.revenueYTD) * 100),
    })), [activeRows]);

  function addMonth() {
    const m = newMonthInput.trim();
    if (!m || payload.months[m]) return;
    const updated: FinancialKPIPayload = {
      ...payload,
      months: { ...payload.months, [m]: allServices.map(EMPTY_KPI_RECORD) },
    };
    setPayload(updated);
    setSelectedMonth(m);
    setNewMonthInput("");
  ...
  function startEdit() {
    setDraftRows(activeRows.map(r => ({ ...r })));
    setEditMode(true);
  }
  ...
  function updateDraft(idx: number, field: keyof FinancialKPIRecord, val: string) {
    setDraftRows(prev => prev.map((r, i) => i === idx ? { ...r, [field]: parseFloat(val) || 0 } : r));
  }

  async function saveToDB(p: FinancialKPIPayload) {
    await fetch("/api/settings/financial-kpi", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleSave() {
    ...
    const updated: FinancialKPIPayload = {
      ...payload,
      months: { ...payload.months, [selectedMonth]: draftRows },
    };
    try {
      const res = await fetch("/api/settings/financial-kpi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function addService() {
    ...
    if (!s || allServices.includes(s)) return;
    const updated: FinancialKPIPayload = {
      ...payload,
      extraServices: [...payload.extraServices, s],
    };
    setPayload(updated);
    setNewServiceInput("");
    saveToDB(updated);
  ...
  const sortedMonths = Object.keys(payload.months).sort((a, b) => b.localeCompare(a));

  // Summary card totals
  const totalMrgYTD = totals.revenueYTD > 0
    ? Math.round(((totals.revenueYTD - totals.costYTD) / totals.revenueYTD) * 100)
    : null;
  const totalTMD = totalMrgYTD !== null ? totalMrgYTD - TMD_TARGET : null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <span className="text-xs font-semibold text-muted-foreground">{t.fkpi_period_label}</span>
        {sortedMonths.length > 0 ? (
          <select
            value={selectedMonth}
            onChange={e => setSelectedMonth(e.target.value)}
            className="text-xs border border-border rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400"
          >
            {sortedMonths.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
        ) : (
          <span className="text-xs text-muted-foreground italic">{t.fkpi_no_month}</span>
        )}
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          <input
            ...
            placeholder={t.fkpi_month_ph}
            className="text-xs border border-border rounded-lg px-3 py-1.5 w-36 focus:outline-none focus:ring-1 focus:ring-indigo-400"
          />
          <button
            onClick={addMonth}
            disabled={!newMonthInput.trim()}
            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-40 transition-colors"
          >
            <Plus className="w-3 h-3" />{t.fkpi_add_month}
          </button>
        </div>
      </div>
      ...
      {!selectedMonth ? (
        <div className="flex items-center justify-center h-48 bg-white rounded-xl border border-border">
          <p className="text-sm text-muted-foreground">{t.fkpi_no_month}</p>
        </div>
      ) : (
        <>
          {/* ── Summary KPI cards ──────────────────────────────────────────── */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl border border-border p-3">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">{t.fkpi_revenue_row} YTD</p>
              <p className="text-sm font-bold text-foreground">${fmt(totals.revenueYTD)}</p>
            </div>
            <div className="bg-white rounded-xl border border-border p-3">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">{t.fkpi_cost_row} YTD</p>
              <p className="text-sm font-bold text-foreground">${fmt(totals.costYTD)}</p>
            </div>
            <div className="bg-white rounded-xl border border-border p-3">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">{t.fkpi_margin_row} YTD</p>
              <p className={`text-sm font-bold ${marginColor(totalMrgYTD)}`}>
                {totalMrgYTD !== null ? `${totalMrgYTD}%` : "—"}
              </p>
            </div>
            <div className="bg-white rounded-xl border border-border p-3">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">{t.fkpi_tmd_row}</p>
              <p className={`text-sm font-bold ${tmdColor(totalTMD)}`}>
                {totalTMD !== null ? `${totalTMD >= 0 ? "+" : ""}${totalTMD}pp` : "—"}
              </p>
            </div>
          </div>

          {/* ── Charts ─────────────────────────────────────────────────────── */}
          {activeRows.some(r => r.revenueYTD > 0) && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Revenue by service */}
              <div className="bg-white rounded-xl border border-border p-4">
                <h3 className="text-[10px] font-bold text-indigo-800 uppercase tracking-wide mb-3">{t.fkpi_chart_revenue}</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={revenueChartData} margin={{ top: 0, right: 8, bottom: 0, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="name" tick={{ fontSize: 10 }} />
                    <YAxis tick={{ fontSize: 10 }} tickFormatter={v => `$${Math.round(v/1000000)}M`} />
                    <ReTT formatter={(v: number) => [`$${fmt(v)}`, ""]} />
                    <Bar dataKey={t.fkpi_col_monthly}    fill={CHART_COLORS[0]} radius={[3,3,0,0]} />
                    <Bar dataKey={t.fkpi_col_ytd}        fill={CHART_COLORS[1]} radius={[3,3,0,0]} />
                    <Bar dataKey={t.fkpi_col_projection} fill={CHART_COLORS[2]} radius={[3,3,0,0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="bg-white rounded-xl border border-border p-4">
                <h3 className="text-[10px] font-bold text-indigo-800 uppercase tracking-wide mb-3">{t.fkpi_chart_margin}</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={marginChartData} margin={{ top: 0, right: 8, bottom: 0, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="name" tick={{ fontSize: 10 }} />
                    <YAxis tick={{ fontSize: 10 }} tickFormatter={v => `${v}%`} />
                    <ReTT formatter={(v: number) => [`${v}%`, t.fkpi_margin_row]} />
                    <ReferenceLine y={TMD_TARGET} stroke="#ef4444" strokeDasharray="4 4" label={{ value: "34%", fontSize: 9, fill: "#ef4444" }} />
                    <Bar dataKey="margin" radius={[3,3,0,0]}>
                      {marginChartData.map((entry, i) => (
                        <Cell key={i} fill={entry.margin >= TMD_TARGET ? "#10b981" : entry.margin >= 25 ? "#f59e0b" : "#ef4444"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <div className="bg-white rounded-xl border border-border overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border">
              <h3 className="text-[10px] font-bold text-indigo-800 uppercase tracking-wide">
                Financial KPIs — {selectedMonth}
              </h3>
              <div className="flex items-center gap-2">
                {saveStatus === "saved" && <span className="text-[10px] text-emerald-600 font-semibold">{t.fkpi_saved_ok}</span>}
                {saveStatus === "error" && <span className="text-[10px] text-red-600 font-semibold">{t.fkpi_save_error}</span>}
                {!editMode ? (
                  <button onClick={startEdit} className="flex items-center gap-1 px-3 py-1.5 text-xs border border-indigo-200 rounded-lg text-indigo-700 hover:bg-indigo-50 transition-colors">
                    <Pencil className="w-3 h-3" />{t.fkpi_edit_data}
                  </button>
                ) : (
                  <>
                    <button onClick={cancelEdit} className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 transition-colors">
                      {t.fkpi_cancel}
                    </button>
                    <button onClick={handleSave} disabled={saveStatus === "saving"} className="px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-60 transition-colors">
                      {saveStatus === "saving" ? "…" : t.fkpi_save_data}
                    </button>
                  </>
                )}
              </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-indigo-50/60 border-b border-indigo-100">
                    <th className="px-4 py-2 text-left font-semibold text-indigo-800">{t.fkpi_service_col}</th>
                    <th className="px-3 py-2 text-center font-semibold text-indigo-800 w-8">{t.fkpi_col_monthly}</th>
                    <th className="px-3 py-2 text-center font-semibold text-indigo-800 w-8">{t.fkpi_col_ytd}</th>
                    <th className="px-3 py-2 text-center font-semibold text-indigo-800 w-8">{t.fkpi_col_projection}</th>
                  </tr>
                </thead>
                <tbody>
                  {(editMode ? draftRows : activeRows).map((r, idx) => {
                    const { profitM, profitY, profitP, mrgM, mrgY, mrgP, tmdM, tmdY, tmdP } = computeRow(r);
                    const inputCls = "w-full text-right border border-indigo-200 rounded px-2 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-400";
                    return (
                      <Fragment key={r.serviceType}>
                        {/* Service header row */}
                        <tr className="bg-slate-50 border-t border-slate-200">
                          <td colSpan={4} className="px-4 py-1.5 font-semibold text-slate-700 text-[11px]">{r.serviceType}</td>
                        </tr>
                        {/* Revenue */}
                        <tr className="border-b border-slate-100 hover:bg-slate-50/50">
                          <td className="px-4 py-1.5 pl-8 text-slate-600">{t.fkpi_revenue_row}</td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.revenueMonthly} onChange={e => updateDraft(idx, "revenueMonthly", e.target.value)} /> : `$${fmt(r.revenueMonthly)}`}
                          </td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.revenueYTD} onChange={e => updateDraft(idx, "revenueYTD", e.target.value)} /> : `$${fmt(r.revenueYTD)}`}
                          </td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.revenueProjection} onChange={e => updateDraft(idx, "revenueProjection", e.target.value)} /> : `$${fmt(r.revenueProjection)}`}
                          </td>
                        </tr>
                        {/* Cost */}
                        <tr className="border-b border-slate-100 hover:bg-slate-50/50">
                          <td className="px-4 py-1.5 pl-8 text-slate-600">{t.fkpi_cost_row}</td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.costMonthly} onChange={e => updateDraft(idx, "costMonthly", e.target.value)} /> : `$${fmt(r.costMonthly)}`}
                          </td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.costYTD} onChange={e => updateDraft(idx, "costYTD", e.target.value)} /> : `$${fmt(r.costYTD)}`}
                          </td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.costProjection} onChange={e => updateDraft(idx, "costProjection", e.target.value)} /> : `$${fmt(r.costProjection)}`}
                          </td>
                        </tr>
                        {/* Profit */}
                        <tr className="border-b border-slate-100 hover:bg-slate-50/50">
                          <td className="px-4 py-1.5 pl-8 text-slate-600">{t.fkpi_profit_row}</td>
                          <td className="px-3 py-1.5 text-right font-medium">${fmt(profitM)}</td>
                          <td className="px-3 py-1.5 text-right font-medium">${fmt(profitY)}</td>
                          <td className="px-3 py-1.5 text-right font-medium">${fmt(profitP)}</td>
                        </tr>
                        {/* Gross Margin % */}
                        <tr className="border-b border-slate-100 hover:bg-slate-50/50">
                          <td className="px-4 py-1.5 pl-8 text-slate-600">{t.fkpi_margin_row}</td>
                          <td className={`px-3 py-1.5 text-right ${marginColor(mrgM)}`}>{mrgM !== null ? `${mrgM}%` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${marginColor(mrgY)}`}>{mrgY !== null ? `${mrgY}%` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${marginColor(mrgP)}`}>{mrgP !== null ? `${mrgP}%` : "—"}</td>
                        </tr>
                        {/* TMD */}
                        <tr className="border-b border-slate-100 hover:bg-slate-50/50">
                          <td className="px-4 py-1.5 pl-8 text-slate-600">{t.fkpi_tmd_row}</td>
                          <td className={`px-3 py-1.5 text-right ${tmdColor(tmdM)}`}>{tmdM !== null ? `${tmdM >= 0 ? "+" : ""}${tmdM}pp` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${tmdColor(tmdY)}`}>{tmdY !== null ? `${tmdY >= 0 ? "+" : ""}${tmdY}pp` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${tmdColor(tmdP)}`}>{tmdP !== null ? `${tmdP >= 0 ? "+" : ""}${tmdP}pp` : "—"}</td>
                        </tr>
                        {/* FTE Man-days */}
                        <tr className="border-b border-slate-200 hover:bg-slate-50/50">
                          <td className="px-4 py-1.5 pl-8 text-slate-600">{t.fkpi_fte_row}</td>
                          <td className="px-3 py-1.5 text-right text-muted-foreground">—</td>
                          <td className="px-3 py-1.5 text-right">
                            {editMode ? <input type="number" className={inputCls} value={r.fteMandays} onChange={e => updateDraft(idx, "fteMandays", e.target.value)} /> : (r.fteMandays > 0 ? r.fteMandays : "—")}
                          </td>
                          <td className="px-3 py-1.5 text-right text-muted-foreground">—</td>
                        </tr>
                      </Fragment>
                  ...
                  {/* TOTALS row */}
                  {activeRows.length > 1 && (() => {
                    const pM = totals.revenueMonthly - totals.costMonthly;
                    const pY = totals.revenueYTD - totals.costYTD;
                    const pP = totals.revenueProjection - totals.costProjection;
                    const mM = totals.revenueMonthly > 0 ? Math.round((pM/totals.revenueMonthly)*100) : null;
                    const mY = totals.revenueYTD > 0 ? Math.round((pY/totals.revenueYTD)*100) : null;
                    const mP = totals.revenueProjection > 0 ? Math.round((pP/totals.revenueProjection)*100) : null;
                    const tM = mM !== null ? mM - TMD_TARGET : null;
                    const tY = mY !== null ? mY - TMD_TARGET : null;
                    const tP = mP !== null ? mP - TMD_TARGET : null;
                    return (
                      <>
                        <tr className="bg-indigo-50 border-t-2 border-indigo-200 font-semibold">
                          <td colSpan={4} className="px-4 py-1.5 text-[11px] text-indigo-700 font-bold uppercase">{t.fkpi_total_row}</td>
                        </tr>
                        <tr className="bg-indigo-50/60 border-b border-indigo-100">
                          <td className="px-4 py-1.5 pl-8 text-slate-700 font-medium">{t.fkpi_revenue_row}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(totals.revenueMonthly)}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(totals.revenueYTD)}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(totals.revenueProjection)}</td>
                        </tr>
                        <tr className="bg-indigo-50/60 border-b border-indigo-100">
                          <td className="px-4 py-1.5 pl-8 text-slate-700 font-medium">{t.fkpi_cost_row}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(totals.costMonthly)}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(totals.costYTD)}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(totals.costProjection)}</td>
                        </tr>
                        <tr className="bg-indigo-50/60 border-b border-indigo-100">
                          <td className="px-4 py-1.5 pl-8 text-slate-700 font-medium">{t.fkpi_profit_row}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(pM)}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(pY)}</td>
                          <td className="px-3 py-1.5 text-right font-semibold">${fmt(pP)}</td>
                        </tr>
                        <tr className="bg-indigo-50/60 border-b border-indigo-100">
                          <td className="px-4 py-1.5 pl-8 text-slate-700 font-medium">{t.fkpi_margin_row}</td>
                          <td className={`px-3 py-1.5 text-right ${marginColor(mM)}`}>{mM !== null ? `${mM}%` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${marginColor(mY)}`}>{mY !== null ? `${mY}%` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${marginColor(mP)}`}>{mP !== null ? `${mP}%` : "—"}</td>
                        </tr>
                        <tr className="bg-indigo-50/60">
                          <td className="px-4 py-1.5 pl-8 text-slate-700 font-medium">{t.fkpi_tmd_row}</td>
                          <td className={`px-3 py-1.5 text-right ${tmdColor(tM)}`}>{tM !== null ? `${tM >= 0 ? "+" : ""}${tM}pp` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${tmdColor(tY)}`}>{tY !== null ? `${tY >= 0 ? "+" : ""}${tY}pp` : "—"}</td>
                          <td className={`px-3 py-1.5 text-right ${tmdColor(tP)}`}>{tP !== null ? `${tP >= 0 ? "+" : ""}${tP}pp` : "—"}</td>
                        </tr>
                      </>
                    );
                  })()}
          ...
          <div className="bg-white rounded-xl border border-border overflow-hidden">
            <button
              ...
              onClick={() => setShowAddService(o => !o)}
            >
              <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">{t.fkpi_manage_services}</span>
              {showAddService ? <ChevronDown className="w-3 h-3 text-muted-foreground" /> : <ChevronRight className="w-3 h-3 text-muted-foreground" />}
            </button>
            {showAddService && (
              <div className="px-4 pb-4 border-t border-border space-y-3 pt-3">
                <div className="flex flex-wrap gap-1.5">
                  {allServices.map(s => (
                    <span key={s} className="text-[10px] bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-full px-2.5 py-0.5">{s}</span>
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder={t.fkpi_add_service}
                    className="text-xs border border-border rounded-lg px-3 py-1.5 flex-1 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                  />
                  <button
                    onClick={addService}
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 text-xs bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-40 transition-colors"
                  >
                    <Plus className="w-3 h-3" />{t.action_add}
                  </button>
                </div>
              </div>
            )}
